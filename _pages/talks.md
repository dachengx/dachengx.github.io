...
<div class="talk-entry mb-3">
  ...
  {% if talk.url %}<a href="{{ talk.url }}" target="_blank">{{ talk.title }}</a>{% else %}{{ talk.title }}{% endif %}<br>
  <span class="text-muted">
    {% if talk.event_url %}<a href="{{ talk.event_url }}" target="_blank">{{ talk.event }}</a>{% else %}{{ talk.event }}{% endif %}
    &mdash; {{ talk.location }}
  </span>
</div>
{% endfor %}
